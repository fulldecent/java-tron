...
public class DelegatedResourceAccountIndexStore extends
  ...
  public void convert(byte[] address) {
    ...
    for (int i = 0; i < toList.size(); i++) {
      // use index as the timestamp
      this.delegate(address, toList.get(i).toByteArray(), i + 1);
    }

    List<ByteString> fromList = indexCapsule.getFromAccountsList();
    for (int i = 0; i < fromList.size(); i++) {
      // use index as the timestamp
      this.delegate(fromList.get(i).toByteArray(), address, i + 1);
    }
    this.delete(address);
  }
  ...
  public DelegatedResourceAccountIndexCapsule getIndex(byte[] address) {
    ...
    List<DelegatedResourceAccountIndexCapsule> tmpToList = new ArrayList<>();
    this.prefixQuery(key).forEach((k, v) -> {
      tmpToList.add(v);
    });
    tmpToList.sort(Comparator.comparing(DelegatedResourceAccountIndexCapsule::getTimestamp));
    for (DelegatedResourceAccountIndexCapsule toIndex : tmpToList) {
      tmpIndexCapsule.addToAccount(toIndex.getAccount());
    }

    System.arraycopy(toPrefix, 0, key, 0, toPrefix.length);
    System.arraycopy(address, 0, key, toPrefix.length, address.length);
    List<DelegatedResourceAccountIndexCapsule> tmpFromList = new ArrayList<>();
    this.prefixQuery(key).forEach((k, v) -> {
      tmpFromList.add(v);
    });
    tmpFromList.sort(Comparator.comparing(DelegatedResourceAccountIndexCapsule::getTimestamp));
    for (DelegatedResourceAccountIndexCapsule fromIndex : tmpFromList) {
      tmpIndexCapsule.addFromAccount(fromIndex.getAccount());
    }
